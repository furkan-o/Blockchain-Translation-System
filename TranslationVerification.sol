...
contract TranslationVerification {
    ...
    function uploadTranslation(string memory hash) public {
        require(bytes(translations[hash].hash).length == 0, "Translation already exists.");
    ...
    function peerReviewTranslation(string memory hash, bool approve) public {
        require(bytes(translations[hash].hash).length != 0, "Translation not found.");
    ...
    function communityVerifyTranslation(string memory hash, bool approve) public {
        ...
        require(translations[hash].approvals > 0, "Peer review must have at least one approval.");
        require(!translations[hash].hasVoted[msg.sender], "You have already voted.");
